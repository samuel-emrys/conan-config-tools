...
def mock_existing_profile(mock_conan_home):
    ...
    config["settings"] = {
        "os": platform.system(),
        "arch": platform.machine() if platform.machine() != "AMD64" else "x86_64"
    }
...
def test_invalid_setting_is_sanitized_from_profile_when_forced(caplog, mock_conan_home):
    # cct -v --conan-home . set-profile -f -n test -s compiler="msvc -s compiler.libcxx=""
    # Invoke the CLI
    runner = CliRunner()
    result = runner.invoke(cli, ["-v", "--conan-home", mock_conan_home, "set-profile", "-f", "-n", "test", "-s", "compiler=msvc", "-s", "compiler.libcxx="])
...
def test_invalid_setting_throws(caplog, mock_conan_home):
    ...
    # Invoke the CLI
    runner = CliRunner()
    result = runner.invoke(cli, ["-v", "--conan-home", mock_conan_home, "set-profile", "-n", "test", "-s", "compiler=msvc", "-s", "compiler.libcxx=libstdc++11"])
    ...
    assert "'settings.compiler.libcxx' doesn't exist for 'msvc'" in "\n".join([str(r.msg) for r in caplog.records])
